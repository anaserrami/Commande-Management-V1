...
export class AddCommandeComponent implements OnInit {
  ...
  commandeForm = new FormGroup({
    ...
    idEtat: new FormControl('', Validators.required),
  ...
  commandeSubmit() {
    if (this.commandeForm.valid) {
      ...
      const prixCommande = ligneDeCommande.reduce((total, lc) => total + lc.prixTotal, 0);
  ...
  getAllDataProduit() {
    this.service.getAllDataProduit().subscribe((res) => {
      ...
      this.products = this.readData; // Set the selected products to the products array
  ...
  commandeUpdate() {
    console.log(this.commandeForm.value, 'updatedform');
    if (this.commandeForm.valid) {
      this.service.updateDataCommande(this.getparamid, this.commandeForm.value).subscribe((res) => {
        console.log(res, 'res ==>');
        this.successmsg = res.message;
      });
    } else {
