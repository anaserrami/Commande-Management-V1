...
app.get('/pays/:id', (req, res) => {
    let gID = req.params.id;
    let qr = `select * from pays where id = ${gID}`;
    db.query(qr, (err, result) => {
        if(err) console.log(err);
        if(result.length > 0) {
            res.send({
...
app.post('/pays', (req, res) => {
    ...
    let nom = req.body.nom;
    let qr = `insert into pays(nom) values('${nom}')`;
...
app.put('/pays/:id', (req, res) => {
    ...
    let qr = `update pays set nom = '${nom}' where id = ${gID}`;
    db.query(qr, (err, result) => {
        if (err) {
            console.log(err);
            res.send({
...
app.delete('/pays/:id', (req, res) => {
    ...
    let qr = `delete from pays where id = ${roleID}`;
    db.query(qr, (err, result) => {
        if (err) {
            console.log(err);
            res.send({
